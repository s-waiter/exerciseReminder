...
import { Globe, MapPin, Monitor, Calendar as CalendarIcon, FilterX } from 'lucide-react';
...
const AdminWebsiteStats = () => {
  ...
  const fetchData = async (date) => {
    ...
    setLogLoading(!!date); // Only show log loading if filtering
    if (!data) setLoading(true); // Initial load
    ...
    try {
      ...
      // If filtering, only update logs to prevent chart flicker, 
      // but here we get full data structure. 
      // Optimization: Backend could have separate endpoints, but for now full refresh is fine.
  ...
  const handleDotClick = (e) => {
      // Prioritize activeLabel (date string from XAxis) if available
      if (e && e.activeLabel) {
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data?.trend} onClick={handleDotClick} className="cursor-pointer">
              ...
              <Line 
                ...
                dot={{ r: 3, strokeWidth: 1, fill: '#3b82f6' }} // Show small dots by default
                name="浏览量 (PV)" 
              />
              <Line 
                type="monotone" 
                ...
                strokeWidth={2} 
                activeDot={{ r: 6, strokeWidth: 2, stroke: '#fff' }}
                dot={{ r: 3, strokeWidth: 1, fill: '#14b8a6' }} // Show small dots by default
                name="访客数 (UV)" 
              />
            </LineChart>
          </ResponsiveContainer>
        ...
        <div className="p-6 border-b border-slate-700/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
              ...
              {filterDate ? `访问日志 (${filterDate})` : '最新访问日志 (Last 100)'}
            </h2>
            <p className="text-slate-400 text-sm mt-1">
              {filterDate 
                ? `显示 ${filterDate} 的所有访问记录` 
                : '显示最近 100 条访问记录，可点击上方图表或右侧日期筛选特定日期'}
            </p>
          </div>
        ...
        <div className="overflow-x-auto max-h-[600px] relative">
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-900/80 text-slate-400 text-sm uppercase sticky top-0 backdrop-blur-md z-10 shadow-sm">
              <tr>
                <th className="px-6 py-4 font-medium tracking-wider">时间</th>
                <th className="px-6 py-4 font-medium tracking-wider">IP / 地理位置</th>
                <th className="px-6 py-4 font-medium tracking-wider">时长</th>
                <th className="px-6 py-4 font-medium tracking-wider">下载</th>
                {/* Referrer column removed as per request */}
                <th className="px-6 py-4 font-medium tracking-wider">设备</th>
              </tr>
            </thead>
                ...
                <tr>
                  <td colSpan="5" className="text-center py-16">
                    <div className="flex flex-col items-center gap-3 text-slate-500">
                      ...
                      <p>
                        {filterDate ? `${filterDate} 无访问记录` : '暂无日志'}
                      </p>
                    </div>
                  </td>
                </tr>
                ...
                data?.logs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-700/30 transition-colors group">
                    <td className="px-6 py-4 text-slate-400 whitespace-nowrap font-mono text-xs">
                      ...
                      <span className="text-slate-600 ml-2">{new Date(log.visited_at).toLocaleDateString()}</span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-white font-mono text-sm group-hover:text-teal-400 transition-colors">{log.ip_address}</div>
                      <div className="text-xs text-slate-500 mt-0.5">{log.city}</div>
                    </td>
                    <td className="px-6 py-4 text-slate-300">
                        {log.duration_seconds > 0 ? (
                            <span className="bg-slate-700/50 px-2 py-0.5 rounded text-xs font-mono">
                    ...
                    {/* Referrer cell removed */}
                    <td className="px-6 py-4">
                      <div className="text-slate-300 text-sm">{log.os} <span className="text-slate-600">/</span> {log.browser}</div>
