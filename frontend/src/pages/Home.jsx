...
function Home() {
  const { version, loading, requireCode } = useVersionInfo();
  const { trackDownload } = useAnalytics();
  const [isModalOpen, setIsModalOpen] = useState(false);

  const handleDownloadClick = (ver) => {
    // If ver is not provided, use version from hook
    const targetVer = ver || version;
    trackDownload(targetVer);
    
    if (requireCode) {
      setIsModalOpen(true);
    } else {
      // If code not required, trigger download directly via API with public code
      window.location.href = "/api/downloads/file?code=public_access";
    }
  };

  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans selection:bg-teal-500 selection:text-white">
      <Navbar onDownloadClick={() => handleDownloadClick(version)} />
      <main>
        <Hero onDownloadClick={() => handleDownloadClick(version)} />
        <TargetAudience />
        <Features />
        <FAQ />
            ...
            >
               ...
               {loading ? (
                 ...
                 <button 
                   ...
                   className="w-full bg-gradient-to-r from-teal-600 to-teal-500 hover:from-teal-500 hover:to-teal-400 text-white font-bold py-4 px-4 rounded-xl transition-all transform hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2 shadow-lg shadow-teal-900/50 group"
                 >
